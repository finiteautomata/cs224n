...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sent in sents:
            sent_ret = []
            for word in sent:
                word_ret = [self.char2id[c] for c in word]
                word_ret = [self.start_of_word] + word_ret + [self.end_of_word]

                sent_ret.append(word_ret)
            ret.append(sent_ret)
        return ret
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        # Now = (batch_size, max_sentence_length, max_word_length)
